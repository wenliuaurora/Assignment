...
void CheckCode(Code* code,SDL_Simplewin* sw,Turtle* turtle){
	...
	else if (ISSAME(CURRENTWORDS, "IF")) {
        Code_IF(code);
	}
...
void Code_IF(Code* code){
	int op;
	double value1,value2;
	int opnum=0;
	PLUSCURR;
	printf("%s",CURRENTWORDS);
	if(!ISSAME(CURRENTWORDS,"(")){
		fprintf(stderr, "CODE,should be ( after the IF instruction.\n");
	}
	PLUSCURR;
	if(!CodeVARNUM(code,&value1)){
		fprintf(stderr, "CODE,should be VARNUM after the IF instruction.\n");
	}
	PLUSCURR;
	if((op=ISCOMOP(code))==FALSE){
		fprintf(stderr, "CODE,should be opeater after the IF instruction.\n");
		return;
	}
	PLUSCURR;
	if(!CodeVARNUM(code,&value2)){
		fprintf(stderr, "CODE,should be VARNUM after the IF instruction.\n");
	}
	PLUSCURR;
	if(ISSAME(CURRENTWORDS,")")){
		if(ISTRUE(op,value1,value2)){
			PLUSCURR;
			if(ISSAME(CURRENTWORDS,"{")){
				PLUSCURR;
				return;
...
void NextIf(Code* code,int opnum){
	PLUSCURR;
	if(ISSAME(CURRENTWORDS,"{")){
		opnum++;
	}
...
bool ISTRUE(int op,double value1,double value2){
	double sub=0;
	sub=value1-value2;
	if(op==GREATER&&sub>0){
		return TRUE;
	}
	else if(op==GREATERANDEQUAL&&sub>=0){
		return TRUE;
	}
	else if(op==LESS&&sub<0){
		return TRUE;
	}
	else if(op==LESSANDEQUAL&&sub<=0){
		return TRUE;
	}
	else if(op==EQUAL&&sub<MINNUM){
		return TRUE;
	}
	return FALSE;
}
int ISCOMOP(Code* code){
	if(ISSAME(CURRENTWORDS,">=")){
